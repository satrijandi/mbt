...
| `WARN no 'sample_key' declared: sampling hashes all 14 columns, ...` | the local data adapter fell back to hashing every column for sampling or for a random split. Membership then moves whenever the schema does, and the Snowflake and Spark adapters reject this path outright - declare `sample_key` |
