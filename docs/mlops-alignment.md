...
| **Automation** | Dataset materialization is declarative, cached by snapshot, and auto-materializes upstream dependencies (ADR-13) | Training, Optuna tuning, evaluation, and registration all run from one `mbt build` | The scaffold ships six workflows: PR check, prod build, promotion, scheduled retrain, scheduled score, scheduled monitor ([GitOps & CI](gitops.md)) |
...
| Fairness and bias testing | Slice gates provide the mechanism (declare segments, gate per slice), but there are no fairness-specific metrics or protected-attribute primitives |
| Explainability beyond feature importance | Model cards carry normalized-gain feature importance; SHAP-style per-prediction attribution is not built in |
| Cross-validation and multi-family model comparison | Not built in; evaluation is a single pinned held-out split, deliberate for temporal data. Comparing families means parallel specs over a shared dataset, tracked side by side in MLflow |
...
| OpenTelemetry spans | Deferred (v0.1 status NFR-06); observability is events, run results, and the Pushgateway/Prometheus spec |
