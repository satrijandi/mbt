...
A third, wide multi-table cadence (`tag:wide`, SHOW-19) exercises ADR-22: a monthly population spine carrying the customer_id-to-safe_id crosswalk, three feature-history tables joined by different keys (transactions only reach the panel through the crosswalk), the label joined from one calendar month after each snapshot via `time_offset`, LightGBM feature selection committed as a reviewable diff, and sparkling H2O AutoML on the selected columns.
...
`make wide` runs the wide multi-table cadence (SHOW-19, ADR-22): the LightGBM probe over all ~65 joined columns on the dev target, `scripts/select_features.py` rewriting `churn_wide_automl`'s committed include list (the printed `git diff --stat` is the reviewable selection), sparkling AutoML on the cluster, population-form scoring on the DuckDB plane, and `scripts/evidently_report.py` rendering the DS-facing drift report (mbt's own PSI/KS monitors stay the enforcement path).
For the real-world scale this cadence models (~7M rows x up to 2000 columns per feature table), the committed tables are the small deterministic default of `scripts/generate_wide_data.py`; its `--customers`/`--filler-columns` knobs synthesize the same shape as large as your disk allows, and the code paths are identical: joins/sampling push down into the source query, `sample_key: customer_id` hash-samples whole customers per `sample_fraction` (same fraction -> same rows; smaller fractions are subsets), and feature selection cuts the width before AutoML ever sees it.
...
- `test_showcase_wide.py` - the wide multi-table cadence (SHOW-19, ADR-22): the population-spine dataset (per-table join keys + the 1-calendar-month label offset) builds via Spark pushdown against the s3a lake, the LightGBM probe reproduces the committed feature list byte-for-byte, `--vars sample_fraction` panel-samples whole customers with the subset property, sparkling AutoML trains on the selected top-K, the population-form scoring input scores the newest cohort under both shift monitors, its outcomes evaluate exactly once at maturity, and the Evidently report renders.
