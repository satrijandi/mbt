...
- The wide multi-table cadence (SHOW-19, ADR-22), `tags: [wide]`, on its own tables (`scripts/generate_wide_data.py`: a `monthly_population` spine carrying the customer_id-to-safe_id crosswalk, `monthly_labels` recorded one calendar month after each snapshot, and three feature histories joined by DIFFERENT keys - demographics/logins by `(customer_id, snapshot_date)`, transactions by `(safe_id, snapshot_date)`; ~65 joined columns, `--customers/--filler-columns` stress knobs):
  `churn_wide_probe` (lightgbm) exists for its gain-importance export, which `scripts/select_features.py` distills into `churn_wide_automl`'s committed include list between BEGIN/END markers - feature selection as a reviewable diff that slim CI retrains on;
  `churn_wide_automl` (h2o_automl, sparkling on prod) trains on the selected top-K and serves through `scoring/wide_retention_scoring.yml`, whose population-form input joins the same tables minus the label at scoring time; `scripts/evidently_report.py` (evidently pinned in the runner image, never an mbt dep) renders the DS-facing drift report beside mbt's own enforcing monitors.
...
| SHOW-19 | wide multi-table cadence (ADR-22) | The population-spine dataset (per-table join keys, `safe_id` reachable only through the crosswalk, 1-calendar-month label offset) builds via Spark pushdown against the s3a lake; the LightGBM probe reproduces the committed feature list byte-for-byte; `--vars sample_fraction` panel-samples whole customers with the subset property; sparkling AutoML trains on the selected top-K and gates pass; the population-form scoring input scores the newest cohort under both shift monitors; its one-month-later outcomes evaluate exactly once at the monitor anchor; the Evidently report renders |
