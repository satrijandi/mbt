#!/bin/bash
# MBT Integration Test - End-to-End with Advanced Features
# Tests: mbt init, multi-table joins, feature selection, drift detection, temporal windowing
set +e  # Don't exit on errors - check() tracks pass/fail
set -o pipefail  # Propagate pipe exit codes (so $? reflects mbt's exit, not tail's)
...
# Project generated by mbt init
PROJECT_NAME="churn_demo"
TRAINING_PIPELINE="${PROJECT_NAME}_training_v1"
SERVING_PIPELINE="serving_churn_model_v1"
...
# ============================================================
# PHASE 0: Generate Project with mbt init
# ============================================================
echo -e "${YELLOW}Phase 0: Generate Project (mbt init --template)${NC}"
echo ""
...
if [ -d "$PROJECT_DIR" ]; then
    echo "  Cleaning up previous project..."
    rm -rf "$PROJECT_DIR"
fi

# Generate project using mbt init
echo "Step 0a: Generate project with mbt init..."
$MBT init "$PROJECT_NAME" \
    ...
    --seed 42 2>&1 | tail -10
check "mbt init generates project" $?

# Overlay integration-test-specific files
echo "Step 0b: Applying integration test overlays..."
cp "$SCRIPT_DIR/overlays/profiles.yaml" "$PROJECT_DIR/profiles.yaml"
cp "$SCRIPT_DIR/overlays/pyproject.toml" "$PROJECT_DIR/pyproject.toml"
cp "$SCRIPT_DIR/overlays/serving_churn_model_v1.yaml" "$PROJECT_DIR/pipelines/$SERVING_PIPELINE.yaml"
check "Overlay files copied" $?
...
# ============================================================
echo -e "${YELLOW}Phase 1: Local Development Workflow (Advanced Pipeline)${NC}"
echo ""
...
$MBT validate "$TRAINING_PIPELINE" 2>&1 | tail -1
check "Pipeline validation passes" $?

# Step 2: Compile locally
echo "Step 2: Compile training pipeline (dev target)..."
$MBT compile "$TRAINING_PIPELINE" --target dev 2>&1 | tail -3
check "Compilation succeeds" $?

# Step 3: Verify manifest
echo "Step 3: Verify manifest..."
if [ -f "target/$TRAINING_PIPELINE/manifest.json" ]; then
    STEP_COUNT=$(python3 -c "import json; m=json.load(open('target/$TRAINING_PIPELINE/manifest.json')); print(len(m['steps']))")
    if [ "$STEP_COUNT" -eq 7 ]; then
        check "Manifest has 7 steps (load_data, split_data, feature_selection, drift_detection, train_model, evaluate, log_run)" 0
    else
        check "Manifest has 7 steps (got $STEP_COUNT)" 1
    fi
else
...
$MBT run --select "$TRAINING_PIPELINE" --target dev 2>&1 | tail -15
...
if [ -f "target/$TRAINING_PIPELINE/run_results.json" ]; then
    RUN_STATUS=$(python3 -c "import json; print(json.load(open('target/$TRAINING_PIPELINE/run_results.json'))['status'])")
...
echo "Step 10b: Creating tables and loading multi-table data into PostgreSQL..."
# Grant schema permissions (needed for existing containers with old init.sql)
docker compose exec -T postgres psql -U admin -d warehouse -c \
    "GRANT ALL ON SCHEMA public TO mbt_user; ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO mbt_user;" > /dev/null 2>&1
$PYTHON -c "
import pandas as pd
...
# Load CSV files and let pandas infer schema
label_df = pd.read_csv('$PROJECT_DIR/sample_data/label_table.csv')
features_a_df = pd.read_csv('$PROJECT_DIR/sample_data/features_table_a.csv')
features_b_df = pd.read_csv('$PROJECT_DIR/sample_data/features_table_b.csv')

# Convert snapshot_date to datetime
label_df['snapshot_date'] = pd.to_datetime(label_df['snapshot_date'])
features_a_df['snapshot_date'] = pd.to_datetime(features_a_df['snapshot_date'])
features_b_df['snapshot_date'] = pd.to_datetime(features_b_df['snapshot_date'])

# Create tables and load data (if_exists='replace' will create schema automatically)
label_df.to_sql('label_table', engine, if_exists='replace', index=False)
features_a_df.to_sql('features_table_a', engine, if_exists='replace', index=False)
features_b_df.to_sql('features_table_b', engine, if_exists='replace', index=False)

print(f'Created and loaded {len(label_df)} rows into label_table')
print(f'Created and loaded {len(features_a_df)} rows into features_table_a')
print(f'Created and loaded {len(features_b_df)} rows into features_table_b')
" 2>&1
check "Multi-table schema created and data loaded into PostgreSQL" $?

# Step 10c: Verify multi-table data
echo "Step 10c: Verify multi-table data..."
LABEL_COUNT=$(docker compose exec -T postgres psql -U mbt_user -d warehouse -t -c "SELECT COUNT(*) FROM label_table;" 2>/dev/null | tr -d ' ')
if [ "$LABEL_COUNT" -gt 0 ] 2>/dev/null; then
    check "label_table has data ($LABEL_COUNT rows)" 0
else
    check "label_table has data" 1
fi

FEATURES_A_COUNT=$(docker compose exec -T postgres psql -U mbt_user -d warehouse -t -c "SELECT COUNT(*) FROM features_table_a;" 2>/dev/null | tr -d ' ')
if [ "$FEATURES_A_COUNT" -gt 0 ] 2>/dev/null; then
    check "features_table_a has data ($FEATURES_A_COUNT rows)" 0
else
    check "features_table_a has data" 1
fi

FEATURES_B_COUNT=$(docker compose exec -T postgres psql -U mbt_user -d warehouse -t -c "SELECT COUNT(*) FROM features_table_b;" 2>/dev/null | tr -d ' ')
if [ "$FEATURES_B_COUNT" -gt 0 ] 2>/dev/null; then
    check "features_table_b has data ($FEATURES_B_COUNT rows)" 0
else
...
$MBT compile "$TRAINING_PIPELINE" --target prod 2>&1 | tail -3
...
$MBT run --select "$TRAINING_PIPELINE" --target prod 2>&1 | tail -15
TRAIN_EXIT=$?
...
# Uses the prod manifest compiled in Phase 3.
...
$MBT step execute --pipeline "$TRAINING_PIPELINE" --step load_data --target prod --run-id "$STEP_RUN_ID" 2>&1 | tail -3
check "Step executor: load_data succeeds" $?

echo "  Executing split_data..."
$MBT step execute --pipeline "$TRAINING_PIPELINE" --step split_data --target prod --run-id "$STEP_RUN_ID" 2>&1 | tail -3
check "Step executor: split_data succeeds" $?

echo "  Executing feature_selection..."
$MBT step execute --pipeline "$TRAINING_PIPELINE" --step feature_selection --target prod --run-id "$STEP_RUN_ID" 2>&1 | tail -3
check "Step executor: feature_selection succeeds" $?

echo "  Executing drift_detection..."
$MBT step execute --pipeline "$TRAINING_PIPELINE" --step drift_detection --target prod --run-id "$STEP_RUN_ID" 2>&1 | tail -3
check "Step executor: drift_detection succeeds" $?

echo "  Executing train_model..."
$MBT step execute --pipeline "$TRAINING_PIPELINE" --step train_model --target prod --run-id "$STEP_RUN_ID" 2>&1 | tail -3
check "Step executor: train_model succeeds" $?

echo "  Executing evaluate..."
$MBT step execute --pipeline "$TRAINING_PIPELINE" --step evaluate --target prod --run-id "$STEP_RUN_ID" 2>&1 | tail -3
check "Step executor: evaluate succeeds" $?

echo "  Executing log_run..."
$MBT step execute --pipeline "$TRAINING_PIPELINE" --step log_run --target prod --run-id "$STEP_RUN_ID" 2>&1 | tail -3
...
if [ "$MLFLOW_RUN_ID" != "unknown" ] && [ -n "$MLFLOW_RUN_ID" ]; then
    $PYTHON -c "
import yaml
with open('pipelines/$SERVING_PIPELINE.yaml') as f:
    data = yaml.safe_load(f)
data['serving']['model_source']['run_id'] = '$MLFLOW_RUN_ID'
with open('pipelines/$SERVING_PIPELINE.yaml', 'w') as f:
    yaml.dump(data, f, default_flow_style=False, sort_keys=False)
print(f'Updated serving pipeline with MLflow run_id: $MLFLOW_RUN_ID')
" 2>&1
fi

$MBT compile "$SERVING_PIPELINE" --target prod 2>&1 | tail -3
...
$MBT run --select "$SERVING_PIPELINE" --target prod 2>&1 | tail -15
...
$MBT compile "$TRAINING_PIPELINE" --target docker 2>&1 | tail -3
check "Training pipeline compilation (docker target) succeeds" $?

$MBT compile "$SERVING_PIPELINE" --target docker 2>&1 | tail -3
...
if [ -f "$SCRIPT_DIR/generated_dags/${TRAINING_PIPELINE}_dag.py" ]; then
    check "Training DAG file generated" 0
else
    check "Training DAG file generated" 1
fi

if [ -f "$SCRIPT_DIR/generated_dags/${SERVING_PIPELINE}_dag.py" ]; then
...
python3 -c "
import ast, sys
for dag_file in ['$SCRIPT_DIR/generated_dags/${TRAINING_PIPELINE}_dag.py', '$SCRIPT_DIR/generated_dags/${SERVING_PIPELINE}_dag.py']:
    try:
