#!/usr/bin/env bash
# Publish the compiled manifest as the durable prod-state baseline
# (FR-STATE-03): append a commit holding manifest.json to the state branch
# (created on first publish) and push it to the remote.
#
# Git plumbing only: never touches the working tree, the index, or the
# current branch, and needs nothing beyond `contents: write` in CI. Branch
# protection on main does not apply to the state branch, and its history is
# an audit trail of every published baseline. Swap for `aws s3 cp` when
# state moves to object storage (see the mbt gitops docs).
#
...
export GIT_AUTHOR_EMAIL=${GIT_AUTHOR_EMAIL:-mbt-state@users.noreply.github.com}
...
for attempt in 1 2; do
  ...
  if git push --quiet "$remote" "$commit:refs/heads/$branch" 2>/dev/null; then
    echo "publish_state: pushed $manifest to $remote/$branch ($commit)"
    exit 0
  fi
  [ "$attempt" = 1 ] && echo "publish_state: push raced with another publish, retrying" >&2
done

echo "publish_state: could not push to $remote/$branch after retry" >&2
exit 1
