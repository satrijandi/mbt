...
(section 3.5). The report value is AUC on the validation split
(higher-is-better contract); a raising callback aborts training."""
    ...
    return ModelSpec.model_validate(
        {
            ...
            "dataset": "ref('d')",
            "target": "label",
            "hyperparameters": {"n_estimators": n_estimators, "max_depth": 2},
...
def test_train_with_report_streams_validation_auc_per_round() -> None:
    ...
    assert [step for step, _ in seen] == list(range(15))  # one report per round
    assert all(0.0 <= value <= 1.0 for _, value in seen)  # AUC, higher-is-better
...
def test_raising_report_aborts_training_promptly() -> None:
    ...
    with pytest.raises(Pruned):
        XGBoostTrainingAdapter({}).train_with_report(_spec(50), _tuning_handle(), _ctx(), report)
    assert calls == [0, 1, 2]  # the remaining 47 rounds were never trained
